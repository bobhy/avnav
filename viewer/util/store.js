 ...
 */
class Store {
    constructor(name,opt_subStore,opt_ownKeys,opt_blackList) {
        /**
         * @private
        ...
        this.data = {};
        this.name=name;
        this.ownKeys=opt_ownKeys||[];
        this.subStore=opt_subStore;
        this.blackList=opt_blackList||[];
        if (this.subStore && ! this.ownKeys.length){
           sassert("sub store without ownKeys provided");
        }
    }
    ...
    splitKeyList(keyList){
        if (! this.subStore) return [keyList,[]];
        let own=[];
        let sub=[];
        keyList.forEach((key)=>{
            if (this.ownKeys.indexOf(key) >= 0){
                own.push(key);
            }
            else{
                sub.push(key);
    ...
    checkKeys(keys){
        if (! this.blackList.length) return true;
        if (! keys instanceof Array){
            keys=[keys];
        }
        let fail=false;
        for (let i=0;i<keys.length;i++){
            if (this.blackList.indexOf(keys[i]) >= 0){
                sassert("key "+keys[i]+" not allowed for store "+this.name);
                fail=true;
            }
        }
        return !fail;
    ...
    register(callback/*,...*/) {
        ...
        if (!callback) return;
        if (this.subStore){
            let [own,sub]=this.splitKeyList(keys);
            this.subStore.register(callback,sub);
            keys=own;
        }
    ...
    deregister(callback) {
        if (this.subStore){
            this.subStore.deregister(callback);
        }
        let idx = this.findCallback(callback);
    ...
    getData(key, opt_default) {
        if (this.subStore){
            if (this.ownKeys.indexOf(key) < 0){
                return this.subStore.getData(key,opt_default);
            }
        }
        this.checkKeys(key);
    ...
    storeData(key, data, opt_noCallbacks) {
        if (this.subStore && this.ownKeys.indexOf(key) < 0){
            return this.subStore.storeData(key,data,opt_noCallbacks);
        }
        this.checkKeys(key);
    ...
    storeMultiple(data, keyTranslations, opt_noCallbacks, opt_omitUndefined) {
        ...
        for (let k in (keyTranslations !== undefined) ? keyTranslations : data) {
            ...
            if (hasChanged) {
                if (this.subStore && this.ownKeys.indexOf(storeKey) < 0){
                    subChangeKeys.push(storeKey);
                }
                else {
        ...
        if (subChangeKeys.length > 0 && (opt_noCallbacks !== true)) {
            this.subStore.callCallbacks(subChangeKeys, opt_noCallbacks);
        }
        return changeKeys.concat(subChangeKeys);
    }
    ...
    getKeysByPrefix(prefix, opt_simpleValuesOnly) {
        ...
        if (this.subStore){
            return rt.concat(this.subStore.getKeysByPrefix(prefix,opt_simpleValuesOnly));
        }
        return rt;
    }
};
